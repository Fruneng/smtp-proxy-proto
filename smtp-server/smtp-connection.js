...
SMTPConnection.prototype._onClose = function( /* hadError */ ) {
    ...
    this._server.onClose(this.session, function(){
        _this._server.connections.delete(this);
        ...
        _this._server.logger.info('[%s] Connection closed to %s', _this._id, _this.clientHostname || _this.remoteAddress);
    })
};
...
SMTPConnection.prototype._startSession = function() {
    ...
    if (this.session){
        this.session.id = this._id
        this.session.remoteAddress = this.remoteAddress
        this.session.clientHostname = this.clientHostname
        this.session.hostNameAppearsAs = this.hostNameAppearsAs
        this.session.envelope = {
            mailFrom: false,
            rcptTo: []
        }
        this.session.user = user
        this.session.transaction = this._transactionCounter + 1
    } else {
        this.session = {
            id: this._id,
